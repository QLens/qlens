...
- `qlens.coverage`: gate coverage over a test run. It records which gate positions your tests execute and which ones an assertion validates, and reports the gates that ran without any check behind them. `pytest --qlens-cov` prints the report from an ordinary test run.
