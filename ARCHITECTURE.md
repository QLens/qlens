...
**Viewer frontend (`viewer/static/`).** Five ES modules served as-is: no bundler, no framework, no external requests. `draw.js` owns the canvas surfaces (waterfall, probability bars, delta bars) and the OKLCH-to-sRGB colour table the design tokens are authored in; `ui.js` is element construction and the small components; `copy.js` holds every explanatory string in both a plain-language and an expert register; `guide.js` is the reading guide, the reliability notice, and the settings panel; `app.js` is state, layout, and fetching. Canvas rather than SVG at every size, because the waterfall's whole point is staying readable at full resolution and a 400k-cell field is not 400k DOM nodes.
