...
def main(argv: list[str] | None = None) -> int:
    ...
    view.add_argument("source", help="a TraceAct .jsonl file, folder, or SqliteSink .db")
    view.add_argument(
        "--state-dir",
    ...
    if not Path(args.source).exists():
        print(f"qlens: source {args.source!r} does not exist", file=sys.stderr)
        return 1

    from qlens.viewer.server import serve

    server = serve(
        args.source, state_dir=args.state_dir, host=args.host, port=args.port
    )
    ...
    print(f"qlens viewer on {url} (source: {args.source})")
